    ...
    public class ScriptGeneratorMX : IScriptGenerator
    {
        public void GenerateInsertScript(List<City> cities)
        {
            try
            {
                ...
                using (StreamWriter writer = new StreamWriter(filePath + @"\InsertScriptForMX.sql", false))
                {
                    for (int i = 0; i < cities.Count; i++)
                    {
                        writer.WriteLine("IF NOT EXISTS");
                        writer.WriteLine("(Select * from Cities where CityName = '" + cities[i].MXCulture.Trim() + "' , StateCode = '" + cities[i].StateCode.Trim() + "' , AND CountryCode = '" + cities[i].CountryCode.Trim() + ")");
