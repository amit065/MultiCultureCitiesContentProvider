...
{
    public class ScriptGeneratorCO : IScriptGenerator
    {
            ...
            try
            {
                ...
                using (StreamWriter writer = new StreamWriter(filePath + @"\InsertScriptForCO.sql", false))
                {
                    for (int i = 0; i < cities.Count; i++)
                    {
                        ...
                        writer.WriteLine("(Select * from Cities where CityName = '" + cities[i].COCulture.Trim() + "' , StateCode = '" + cities[i].StateCode.Trim() + "' , AND CountryCode = '" + cities[i].CountryCode.Trim() + ")");
                        writer.WriteLine("Begin");
                        writer.WriteLine("Insert into Cities (CityName, StateCode, CountryCode, Latitude, Longitude, IsEnabled, IataCityCode, FullTextColumn) values('" + cities[i].COCulture.Trim() + "' , '" + cities[i].StateCode.Trim() + "' , '"
