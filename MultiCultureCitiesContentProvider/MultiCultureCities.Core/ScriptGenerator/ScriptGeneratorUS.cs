            ...
            try
            {
                ...
                using (StreamWriter writer = new StreamWriter(filePath + @"\InsertScriptForUS.sql", false))
                {
                    for (int i = 0; i < cities.Count; i++)
                    {
                        ...
                        writer.WriteLine("(Select * from Cities where CityName = '" + cities[i].USCulture.Trim() + "' , StateCode = '" + cities[i].StateCode.Trim() + "' , AND CountryCode = '" + cities[i].CountryCode.Trim() + ")");
